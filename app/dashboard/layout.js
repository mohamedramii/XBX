...
const DashboardLayout = ({ children }) => {
  ...
  return (
    <div className="flex min-h-screen w-full">
        ...
        <nav className="flex flex-col gap-[8px] sm:gap-[12px] px-4 sm:px-10">
          {menuItems.map((item, index) => 
            item === null ? (
              ...
              >
                <span className="text-[14px] sm:text-base text-[#787777]">
                  {item.title}
                </span>
              </div>
            ) : (
              <Link
                ...
                className={`block text-base sm:text-xl relative ${
                  (pathname.includes(item.path) && item.path === '/dashboard/products') || pathname === item.path
      ...
      {/* Main Content */}
      <div className={`transition-all duration-300 ease-in-out flex-1 flex flex-col h-screen ${
        ...
        <header className="h-[70px] sm:h-[90px] bg-white px-4 sm:px-[34px] flex justify-between items-center shrink-0">
          <div className="flex items-center gap-[10px] sm:gap-[30px]">
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="w-8 h-8 sm:w-10 sm:h-10">
              <Image 
          ...
          <div className="flex items-center gap-3 sm:gap-5">
            <span className="text-sm sm:text-2xl font-semibold">Hi, {adminUser?.name || 'Admin'}</span>
            ...
            <button 
              ...
              className="w-8 h-8 sm:w-10 sm:h-10 text-[#C30000]"
            >
              <Image
