      ...
      <div className="flex flex-col items-start gap-4 sm:gap-[25px] w-full">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center w-full gap-4 sm:gap-0">
          <div className="flex flex-row items-center justify-center gap-4 sm:gap-[20px]">
            {/* Delivered Option */}
            <div className="flex items-center gap-2 sm:gap-[10px] cursor-pointer" onClick={() => handleStatusChange('Delivered')}>
              <div 
                className={`w-[15px] sm:w-[20px] h-[15px] sm:h-[20px] bg-[#FBFBFB] border-2 border-[#11A900] relative ${
                  selectedStatus === 'Delivered' ? 'after:content-[""] after:absolute after:w-[8px] sm:after:w-[12px] after:h-[8px] sm:after:h-[12px] after:bg-[#11A900] after:left-[2px] sm:after:left-[2px] after:top-[2px] sm:after:top-[2px]' : ''
                }`}
              />
              <span className="font-poppins font-semibold text-[14px] sm:text-[18px] text-[#061404]">Delivered</span>
            </div>

            {/* Cancelled Option */}
            <div className="flex items-center gap-2 sm:gap-[10px] cursor-pointer" onClick={() => handleStatusChange('Cancelled')}>
              <div 
                className={`w-[15px] sm:w-[20px] h-[15px] sm:h-[20px] bg-[#FBFBFB] border-2 border-[#11A900] relative ${
                  selectedStatus === 'Cancelled' ? 'after:content-[""] after:absolute after:w-[8px] sm:after:w-[12px] after:h-[8px] sm:after:h-[12px] after:bg-[#11A900] after:left-[2px] sm:after:left-[2px] after:top-[2px] sm:after:top-[2px]' : ''
                }`}
              />
              <span className="font-poppins font-semibold text-[14px] sm:text-[18px] text-[#061404]">Cancelled</span>
            </div>
          </div>
