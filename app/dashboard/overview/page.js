    ...
    <div className="space-y-5 sm:space-y-[30px]   py-4  w-full max-w-[1440px] ">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
        {stats.map((stat, index) => (
          <div
            key={index}
            className="bg-white p-4 sm:p-[26px_20px] rounded-[20px] shadow-[0px_5px_10px_rgba(0,0,0,0.1)]"
          >
            <div className="flex justify-between items-center mb-2.5">
              <div>
                <p className="text-[12px] sm:text-[13px] font-semibold text-[#8A8A8A] mb-1">
                  {stat.label}
                </p>
                <p className="text-[24px] sm:text-[30px] font-semibold text-[#061404]">
                  {stat.value}
                </p>
              </div>
              <div className="w-[50px] sm:w-[74px] h-[50px] sm:h-[74px] flex items-center justify-center bg-[rgba(17,169,0,0.1)] rounded-[20px]">
                <Image src={stat.icon} width={30} height={30} alt={stat.label} />
              </div>
            </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-[#B7B7B7]">
                {['userName', 'Category', 'OrderDate', 'OrderTime', 'TotalPrice', 'paid'].map(
                  (header) => (
                    <th
                      key={header}
                      className="py-2 sm:py-[15px] px-2 sm:px-[10px] text-base sm:text-xl font-medium text-[#061404] capitalize"
                    >
                      {header}
                    </th>
                  )
            ...
            <tbody>
              {orders.map((order, index) => (
                <tr key={index} className="border-b border-[#B7B7B7] last:border-none">
                  <td className="py-2 sm:py-[15px] px-2 sm:px-[10px] text-[14px] sm:text-xl text-[#061404]">
                    {order.userName}
                  </td>
                  <td className="py-2 sm:py-[15px] px-2 sm:px-[10px] text-[14px] sm:text-xl text-[#061404]">
                    {order.category}
                  ...
                  <td className="py-2 sm:py-[15px] px-2 sm:px-[10px] text-[14px] sm:text-xl text-[#11A900]">
